...
module KinnexusTestKit
  module KinnexusV001
    class ImmunizationGroup < Inferno::TestGroup
      ...
      short_description 'Verify support for the server capabilities required by the Kinnexus Immunization.'
      description %(
  # Background

The Kinnexus Immunization sequence verifies that the system under test is
able to provide correct responses for Immunization queries. These queries
must contain resources conforming to the Kinnexus Immunization as
...
the [Kinnexus Immunization](https://kinnexus.org/StructureDefinition/KinnexusImmunization). Each element is checked against
...
read succeeds.
      ...
      test from: :kinnexus_v001_immunization_reference_resolution_test
    end
  end
end
