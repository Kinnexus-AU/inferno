...
module KinnexusTestKit
  module KinnexusV001
    class AllergyIntoleranceGroup < Inferno::TestGroup
      title 'Allergy Intolerance Tests'
      short_description 'Verify support for the server capabilities required by the Kinnexus Allergy Intolerance.'
      description %(
  # Background

The Kinnexus Allergy Intolerance sequence verifies that the system under test is
able to provide correct responses for AllergyIntolerance queries. These queries
must contain resources conforming to the Kinnexus Allergy Intolerance as
...
the [Kinnexus Allergy Intolerance](https://kinnexus.org/StructureDefinition/KinnexusAllergyIntolerance). Each element is checked against
...
read succeeds.
      ...
      id :kinnexus_v001_allergy_intolerance
      run_as_group
      ...
      test from: :kinnexus_v001_allergy_intolerance_reference_resolution_test
    end
  end
end
