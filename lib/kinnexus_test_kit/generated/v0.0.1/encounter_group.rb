...
module KinnexusTestKit
  module KinnexusV001
    class EncounterGroup < Inferno::TestGroup
      ...
      short_description 'Verify support for the server capabilities required by the Kinnexus Encounter.'
      description %(
  # Background

The Kinnexus Encounter sequence verifies that the system under test is
able to provide correct responses for Encounter queries. These queries
must contain resources conforming to the Kinnexus Encounter as
...
the [Kinnexus Encounter](https://kinnexus.org/StructureDefinition/KinnexusEncounter). Each element is checked against
...
read succeeds.
      ...
      test from: :kinnexus_v001_encounter_reference_resolution_test
    end
  end
end
