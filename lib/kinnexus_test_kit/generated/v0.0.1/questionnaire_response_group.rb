...
module KinnexusTestKit
  module KinnexusV001
    class QuestionnaireResponseGroup < Inferno::TestGroup
      ...
      short_description 'Verify support for the server capabilities required by the Kinnexus QuestionnaireResponse.'
      description %(
  # Background

The Kinnexus QuestionnaireResponse sequence verifies that the system under test is
able to provide correct responses for QuestionnaireResponse queries. These queries
must contain resources conforming to the Kinnexus QuestionnaireResponse as
...
the [Kinnexus QuestionnaireResponse](https://kinnexus.org/StructureDefinition/KinnexusQuestionnaireResponse). Each element is checked against
